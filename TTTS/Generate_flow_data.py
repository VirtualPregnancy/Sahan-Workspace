...
placenta_filename = 'placenta_' + sample_number + '.png'
...
inlets, anast_node, anast_elem, art_nodes, art_elems = skel2graph(skel_art, outputfilename, debug_export_all, inlet_type)
inlet_nodes = []
inlet_nodes_real = []
for inlet in inlets:
    closest_node = [len(inlet_nodes),inlet[1], inlet[0],0]
    closest_real_node, closest_elem = find_closest_node(inlet[1], inlet[0],art_nodes,art_elems, True)
    inlet_nodes.append(closest_node)
    inlet_nodes_real.append(closest_real_node)
inlet_nodes = np.asarray(inlet_nodes)

inlet_nodes_real = np.asarray(inlet_nodes_real)


if is_TTTS:
    removed_element = anast_elem[0,0]
    modified_elems = np.delete(art_elems, removed_element, axis=0)
    for elem_number in range(removed_element,len(modified_elems)):
        modified_elems[elem_number,0] -= 1
    for nodes in inlet_nodes_real:
        pg.fix_elem_direction(nodes[1:4], modified_elems, art_nodes)

    N, E = find_root_nodes(art_nodes, modified_elems)
    art_elems = modified_elems
...
if inlet_node:
    nodes_Umb, elems_Umb = create_umb_anastomosis(arterial_shaped_nodes, art_elems, umbilical_length, outputfilename,
                                                  debug_export_all, inlet_type, None, None)
...
if is_TTTS:
    root_nodes, root_elems = find_root_nodes(nodes_Umb, elems_Umb)
    pg.export_ex_coords(root_nodes, 'arteries', output_tree_dir + 'test2', 'exnode')

    modified_elems = np.insert(elems_Umb, anast_elem[0,0], anast_elem[0,:], axis=0)
    for elem_number in range(anast_elem[0,0]+1,len(modified_elems)):
        modified_elems[elem_number,0] +=1

    if modified_elems[anast_elem[0,0],:][2] == modified_elems[anast_elem[0,0]+1,:][2]: #Fail safe check
        anast_node_num = modified_elems[anast_elem[0,0],:][2]

        new_node_x = nodes_Umb[anast_node_num, 1]
        new_node_y = nodes_Umb[anast_node_num, 2]
        new_node_z = nodes_Umb[anast_node_num, 3] - sv_length
        new_node = np.asarray([len(nodes_Umb),new_node_x,new_node_y,new_node_z])
        new_node = new_node.reshape(1, 4)
        anast_nodes = np.vstack([nodes_Umb,new_node])
        new_elem = np.asarray([len(modified_elems),anast_node_num,int(new_node[0][0])])
        new_elem = new_elem.reshape(1,3)
        anast_elems = np.vstack([modified_elems,new_elem])
        pg.export_ex_coords(anast_nodes, 'arteries', output_tree_dir + 'AN', 'exnode')
        pg.export_exelem_1d(anast_elems, 'arteries', output_tree_dir + 'AE')

        nodes_Umb, elems_Umb = create_umb_anastomosis(anast_nodes, anast_elems, umbilical_length,
                                                      outputfilename,
                                                     debug_export_all, inlet_type, root_nodes,root_elems)

        elems_test, elem_mod, anast_elem = element_rearrage(nodes_Umb, elems_Umb, new_elem[0,0]+3)
        #pg.export_exelem_1d(elems_test, 'arteries', output_tree_dir + 'C1')
        elems_Umb = elem_mod
        pg.export_exelem_1d(elems_Umb, 'arteries', output_tree_dir + 'C')
        pg.export_ex_coords(nodes_Umb, 'arteries', output_tree_dir + 'C', 'exnode')
terminal = pg.calc_terminal_branch(nodes_Umb[:, 1:4], elems_Umb)

branch_structure, branch_data = allocate_branch_numbers(nodes_Umb, elems_Umb, anast_elem[0])
pg.export_exfield_1d_linear(branch_structure, 'arteries', 'branch', output_tree_dir + 'branch')
chorion_nodes, chorion_elems = add_stem_villi(nodes_Umb, elems_Umb, sv_length, terminal, anast_elem[0])
...
N2,E2 = find_root_nodes(chorion_nodes,chorion_elems)
...
elem_cnct_shaped = element_connectivity_multi(chorion_nodes[:, 1:4], chorion_elems)
...
full_geom_shaped = pg.grow_large_tree(angle_max_ft, angle_min_ft, fraction_ft, min_length_ft, point_limit_ft, volume,
                                      thickness, 0, datapoints_ellipse_array, chorion_and_stem_shaped, 1, parent_list_elems)
